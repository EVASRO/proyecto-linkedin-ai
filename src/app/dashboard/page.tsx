...
async function DashboardContent() {
  const [metricsResult, onboardingResult] = await Promise.all([
    getDashboardMetrics(),
    getOnboardingStatus(),
  ]);
    ...
    <>
      {onboarding && !onboarding.completed && (
        <div className="px-6 pt-6">
          <OnboardingChecklist initialSteps={onboarding.steps} />
        </div>
      )}
      <DashboardView metrics={metricsResult.data ?? null} />
    </>
  );
}
    ...
    <div className="flex flex-1 flex-col overflow-hidden min-h-0">
      ...
      <div className="flex flex-shrink-0 items-center justify-between border-b border-[var(--border)] bg-[var(--surface)] px-6 py-4">
        <div className="space-y-1.5">
          ...
          <div className="h-3 w-48 animate-pulse rounded bg-[var(--border)] opacity-70" />
        </div>
      ...
      <div className="flex-1 overflow-y-auto bg-[var(--background)] p-6 space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 shadow-sm">
              <div className="flex items-start justify-between">
                <div className="space-y-2 flex-1">
                  ...
                  <div className="h-2.5 w-24 animate-pulse rounded bg-[var(--border)] opacity-70" />
                </div>
                <div className="h-10 w-10 flex-shrink-0 animate-pulse rounded-xl bg-[var(--border)]" />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
          ...
          <div className="space-y-6">
            <div className="h-52 animate-pulse rounded-2xl bg-[var(--border)]" />
